...
This project includes an analysis of several advertising campaign results of big consumer brands on amazon.com, with a goal to look for potential ways of optimization to improve ROIs.
...
I looked into ROI by campaign types, keeping in mind that average ROI for all campaigns is 2.875, and found Sponsored Brands campaigns have ROI 7.056!
...
We can also optimize campaigns via adjusting ad placements.  
...
My goal was to analyze each keyword performance dynamically. 
For this part, I used plotly library.

### Keyword performance

The charts below show the performance for a search term 'hand sanitizer' across all campaigns.
...
I performed high-level analytics of data grouped by campaign.
...
The graph below shows the two-week hand sanitizer sales forecast, overall trend, and weekly fluctuation component.
...
Each feature has different affect on the outcome (sales). A high-level view can be drawn from the model itself, by plotting these feature importance parameters:
```
...
This graph shows number of clicks and money spent are the most important (which makes sense), but how exactly do they affect sales? To understand the model behavior better, I looked into SHAP values.

SHAP values show the importance of each feature by comparing what a model predicts with and without the feature. In other words, each SHAP value measures how much each feature in our model contributes, either positively or negatively, to each prediction.
...
SHAP summary plot shows the positive and negative relationships of the predictors with the target variable (sales) for each factor. Each dot represents one observation of training data. Below is a short graph explanation. 
...
In our case, the change of cost clicks doesn't seem to affect the outcome (sales) much, though there is a slight linear dependence in between them. 
...
The plot below shows simple interpretation of all feature mportance’s and effects. The red color means a feature is positively correlated with the target variable.
...
## Next step:
Moving on, I'd like to create an interactive webpage with the features described above to provide clear and detailed information about each advertising campaign, search term or product.
